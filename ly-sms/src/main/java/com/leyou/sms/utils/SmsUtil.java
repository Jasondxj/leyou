...
public class SmsUtil {
    ...
    public SendSmsResponse sendSms(String phoneNumber,
        ...
        //读取上一次发送短信时间,按手机号限流,1分钟内只能获取一次验证码
        ...
        try {
            ...
            //必填:待发送手机号（接收方手机号）
//        request.setPhoneNumbers("13600527634");
            ...
            request.setTemplateParam(paramTemplate);
...
//        request.setOutId("123456");
            ...
            if (!"OK".equals(sendSmsResponse.getCode())) {
                log.info("[短信服务]短信发送失败，phoneNumber:{},原因:{}", phoneNumber, sendSmsResponse.getMessage());
            }
            //发送短信日志
            log.info("[短信服务]，发送短信验证码，手机号:{}", phoneNumber);
            //发送短信成功，将时间写入redis，设置时长为1分钟
            redisTemplate.opsForValue().set(key, String.valueOf(System.currentTimeMillis()), 1, TimeUnit.MINUTES);
            return sendSmsResponse;
        } catch (Exception e) {
            log.error("[短信服务]短信发送异常,手机号码{}", phoneNumber, e);
            return null;
        }
    }
